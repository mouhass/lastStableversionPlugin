...
class CommandModifJob extends Command
{
    protected static $defaultName = 'app:modifJob';
    private $mailer;
    private $logger;
    private $kernel;
    protected $manager;
    ...
    {   //******
        ...
        try {
            ...
            $log = "command name: app:modifJob state: success  execution date" . ' - ' . date("F j, Y, G:i") . PHP_EOL .
                "-------------------------" . PHP_EOL;


            if ($input->getArgument('code_job_composite') == "0") {
                file_put_contents($this->kernel->getProjectDir() . '/var/log/modifJob_succes' . date("y-m-d-G-i-s") . '.log', $log, FILE_APPEND);
            } else {
                file_put_contents($this->kernel->getProjectDir() . '/var/log/modifJob_succes' . $input->getArgument('code_job_composite') . date("y-m-d-G-i-s") . '.log', $log, FILE_APPEND);
           ...
           $jobCronModif = $this->repository->findElementById(5);
            $jobCronModif->setName($jobCron->getName()." modifié");
            ...
            $log = "command name: app:modifJob  state: error  error date" . ' - ' . date("F j, Y, G:i") . PHP_EOL .
                "error description : ".$exception.
                "-------------------------" . PHP_EOL;
            file_put_contents($this->kernel->getProjectDir() . '/var/log/modifJob_error' . date("y-m-d-G-i-s") . '.log', $log, FILE_APPEND);
